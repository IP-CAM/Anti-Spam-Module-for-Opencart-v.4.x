...
class AntispamByCleantalk extends \Opencart\System\Engine\Controller
{
    public function index()
    {
        ...
        if ( $this->extension_antispambycleantalk_core->rc->check(
            $this->config->get('module_antispambycleantalk_access_key')
        ) ) {
            $this->extension_antispambycleantalk_core->rc->perform(
                $this->config->get('module_antispambycleantalk_access_key')
            );
        } elseif ( $this->request->server['REQUEST_METHOD'] === 'GET' ) {
            $this->extension_antispambycleantalk_core->sfw->run(
                $this->config->get('module_antispambycleantalk_access_key')
            );
            ...
            if (
                $this->config->get('module_antispambycleantalk_int_sfw_last_check') &&
                time() - $this->config->get('module_antispambycleantalk_int_sfw_last_check') > 86400
            ) {
                $this->extension_antispambycleantalk_core->sfw->sfwUpdate($this->config->get('module_antispambycleantalk_access_key'));
                $settings_array['module_antispambycleantalk_int_sfw_last_check'] = time();
            }

            if (
                $this->config->get('module_antispambycleantalk_int_sfw_last_send_logs') &&
                time() - $this->config->get('module_antispambycleantalk_int_sfw_last_send_logs') > 3600
            ) {
                $this->extension_antispambycleantalk_core->sfw->logsSend($this->config->get('module_antispambycleantalk_access_key'));
            ...
            if ( count($settings_array) > 0 ) {
                foreach ( $settings_array as $setting_name => $setting_value ) {
                    $this->db->query(
                        'UPDATE `' . DB_PREFIX . 'setting` SET `value` = "' . $setting_value . '" WHERE `key` = "' . $setting_name . '"'
                    );
                }
            }
        }
